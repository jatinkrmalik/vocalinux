...
class TestRecognitionState:
    ...
    def test_recognition_state_enum_members(self):
        ...
        states = [RecognitionState.IDLE, RecognitionState.LISTENING, 
                  RecognitionState.PROCESSING, RecognitionState.ERROR]
        assert len(states) == 4
...
class TestProtocols:
    ...
    def test_protocols_have_required_methods(self):
        ...
        assert hasattr(SpeechRecognitionManagerProtocol, '__protocol_attrs__') or \
               hasattr(SpeechRecognitionManagerProtocol, '__mro__')
        
        # TextInjectorProtocol should have required methods
        assert hasattr(TextInjectorProtocol, '__protocol_attrs__') or \
               hasattr(TextInjectorProtocol, '__mro__')
