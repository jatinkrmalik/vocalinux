...
class TestSettingsDialogInstantApply(unittest.TestCase):
    ...
    def test_restart_now_button_has_custom_response_handler(self):
        ...
        self.assertIn("self._cancel_gpu_restart_countdown()", source_code)
        self.assertIn('"vocalinux.restart_helper"', source_code)
...
class TestSettingsDialogHelperFunctions(unittest.TestCase):
    ...
    def test_get_detected_gpu_options_filters_unsupported_backends(self):
        """Test GPU selector omits detected devices for unsupported compiled backends."""
        import vocalinux.ui.settings_dialog as settings_dialog
        ...
        with (
            ...
            options = settings_dialog._get_detected_gpu_options(allowed_backends={"cuda"})

        self.assertFalse(any(option["gpu_backend"] == "vulkan" for option in options))
        self.assertTrue(any(option["gpu_backend"] == "cuda" for option in options))
