...
currently supporting VOSK, Whisper, and whisper.cpp with Vulkan acceleration.
"""
...
# Import whisper.cpp engine if available
try:
    from .whisper_cpp import WhisperCppEngine, create_whisper_cpp_engine
    WHISPER_CPP_AVAILABLE = True
except ImportError:
    WHISPER_CPP_AVAILABLE = False
    WhisperCppEngine = None
    create_whisper_cpp_engine = None
...
class SpeechRecognitionManager:
    ...
    speech recognition engines (VOSK, Whisper, and whisper.cpp with Vulkan).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            engine: The speech recognition engine to use ("vosk", "whisper", or "whisper.cpp")
        ...
        self.whisper_cpp_engine = None  # Added for whisper.cpp
    ...
    def _init_whisper_cpp(self):
        ...
        if not WHISPER_CPP_AVAILABLE:
            error_msg = "whisper.cpp engine not available. Install with: GGML_VULKAN=1 pip install git+https://github.com/abdeladim-s/pywhispercpp"
            logger.error(error_msg)
            self.state = RecognitionState.ERROR
            raise ImportError(error_msg)

        try:
            # Map model sizes to whisper.cpp format
            ...
            # Get the correct model size for whisper.cpp
            whisper_cpp_model = whisper_cpp_model_map.get(self.model_size, "base.en")
            
            # Initialize whisper.cpp engine
            self.whisper_cpp_engine = WhisperCppEngine(
                model_size=whisper_cpp_model,
                language=self.language,
                n_threads=4,
                use_gpu=True,
            )
            
            logger.info(f"whisper.cpp engine initialized with model: {whisper_cpp_model}")
    ...
    def register_text_callback(self, callback: Callable[[str], None]):
        ...
        # Also register with whisper.cpp engine if available
        if self.whisper_cpp_engine:
            self.whisper_cpp_engine.register_text_callback(callback)
    ...
    def register_state_callback(self, callback: Callable[[RecognitionState], None]):
        ...
        # Also register with whisper.cpp engine if available
        if self.whisper_cpp_engine:
            self.whisper_cpp_engine.register_state_callback(callback)
    ...
    def model_ready(self) -> bool:
        ...
        if self.engine == "whisper.cpp":
            return self.whisper_cpp_engine and self.whisper_cpp_engine.is_initialized
        else:
    ...
    def start_recognition(self):
        ...
        # For whisper.cpp, use its real-time transcription
        if self.engine == "whisper.cpp" and self.whisper_cpp_engine:
            self.whisper_cpp_engine.start_real_time_transcription()
        else:
    ...
    def stop_recognition(self):
        ...
        # For whisper.cpp, stop its real-time transcription
        if self.engine == "whisper.cpp" and self.whisper_cpp_engine:
            self.whisper_cpp_engine.stop_real_time_transcription()
        else:
    ...
    def _process_final_buffer(self):
        ...
        elif self.engine == "whisper.cpp" and self.whisper_cpp_engine:
            try:
                # Process audio data with whisper.cpp engine
                audio_bytes = b"".join(self.audio_buffer)
                text = self.whisper_cpp_engine.transcribe_audio_data(audio_bytes)
            except Exception as e:
                logger.error(f"Error processing audio with whisper.cpp: {e}")
                text = ""
    ...
    def reconfigure(
        ...
        **kwargs,  # Allow for future expansion
        ...
        Args:
            engine: The new speech recognition engine ("vosk", "whisper", or "whisper.cpp").
        ...
        # Language change requires restart for all engines
