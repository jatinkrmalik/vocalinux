...
class StreamingSpeechRecognizer:
        ...
        while self.is_running:
            try:
                ...
                if self.total_chunks_processed > 0:
                    self.avg_latency_ms = (self.total_processing_time / self.total_chunks_processed) * 1000
    ...
    def _process_whisper_chunk(self, audio_chunk: bytes):
        ...
        if len(self.speech_buffer) > 0:
            ...
            total_samples = sum(len(chunk) // 2 for chunk in self.speech_buffer)  # 16-bit = 2 bytes per sample
