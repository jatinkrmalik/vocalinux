...
class VisualFeedbackIndicator:
    ...
    def _create_overlay_window(self):
        ...
        try:
            ...
            # Set size
            self.overlay_window.set_default_size(
                self.indicator_size * 2, self.indicator_size * 2
            )
            self.overlay_window.set_size_request(
                self.indicator_size * 2, self.indicator_size * 2
            )
    ...
    def _on_draw(self, widget, cr):
        ...
        scale = self.pulse_min_scale + (
            (self.pulse_max_scale - self.pulse_min_scale) 
            * (math.sin(self.animation_phase) + 1) / 2
        )
        ...
        cr.arc(center_x - radius * 0.2, center_y - radius * 0.2, highlight_radius * 0.3, 0, 2 * math.pi)
        cr.fill()
    ...
    def _update_position(self):
        ...
        if (self.last_cursor_position is None or 
            abs(current_pos.x - self.last_cursor_position.x) > 5 or 
            abs(current_pos.y - self.last_cursor_position.y) > 5):
